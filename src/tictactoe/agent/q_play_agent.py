...
class QPlayAgent(Agent):
    class QAgentFactory(AgentFactory):
        ...
        def new_x_agent(self, q_vals_filename: str) -> Agent:
            """
            Create an X Agent from the given q values
            :param q_vals_filename: The filename of the q values as JSON
            :return: QAgent as Player X
            """
            return QPlayAgent(agent_id=Agent.X_ID, agent_name=Agent.X_NAME, filename=q_vals_filename)

        def new_o_agent(self, q_vals_filename: str) -> Agent:
            """
            Create an O Agent from the given q values
            :param q_vals_filename: The filename of the q values as JSON
            :return: QAgent as Player O
            """
            return QPlayAgent(agent_id=Agent.O_ID, agent_name=Agent.O_NAME, filename=q_vals_filename)
